...
def test_limit_words_keeps_200_words_unchanged():
    text = make_words(200)
    assert limit_words(text) == text


def test_limit_words_truncates_201_words():
    result = limit_words(make_words(201))
    assert len(result.removesuffix("...").split()) == 200
    assert result.endswith("...")


def test_limit_words_supports_vietnamese_whitespace_words():
    text = " ".join(["di tích"] * 101)
