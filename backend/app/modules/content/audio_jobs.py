...
def find_audio_gaps(db) -> list[AudioTarget]:
    ...
    for item_id, persona, language, text_content, audio_mime, audio_missing in rows:

        if is_no_information_content(text_content):
...
def ensure_audio_parallel(
    ...
    unique = filter_backoff_targets(unique)
    ...
    workers = resolve_parallel_workers(max_workers or CONTENT_TTS_MAX_WORKERS)
