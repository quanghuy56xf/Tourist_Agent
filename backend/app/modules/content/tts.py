...
async def _synthesize_speech_async(text: str, language: str, persona: str | None = None) -> tuple[bytes, str]:
...
def synthesize_speech(
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as pool:
            future = pool.submit(
                asyncio.run, _synthesize_speech_async(cleaned, language, persona)
            )
